...
export default function ProductsCarousel() {
  ...
  return (
    <section className="relative w-[94vw] overflow-hidden text-center mx-auto">
      {/* Judul */}
      ...
      <div ref={emblaRef} className="overflow-hidden w-screen">
        <div className="flex">
          {images.map((src, i) => (
            <div key={i} className="shrink-0 w-screen flex justify-center">
              <Image
                src={src}
                width={800}
                height={600}
                alt={`Product ${i + 1}`}
                className="object-contain w-auto max-h-[70vh]"
              />
            </div>
          ))}
        </div>
